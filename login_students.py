...
def login():
    ...
    cursor.execute("SELECT email, password,student_id FROM students WHERE email=:email", {'email': username})
    ...
    if username == '' or password == '':
        ...
        #here update all student to 0 and current to 1
        cursor.execute("UPDATE students SET current_user = 0 WHERE student_id <> :student_id",
                       {'student_id': current_user_id})
        cursor.execute("UPDATE students SET current_user = 1 WHERE student_id = :student_id",
                       {'student_id': current_user_id})
        conStr.commit()
...
if __name__=='__main__':
    ...
    forgotPasswordButton = Button(loginFrame, text='Mot de passe oublié?', font=('times new roman', 12), fg='white',
                                  bg='#0072C6', activebackground='#0072C6', activeforeground='white', cursor='hand2',
